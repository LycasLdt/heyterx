...
import type { ReportType, UserPreferences } from "./db/schema";
import { writeCoreMemory } from "@/lib/memory";

// 领域类型与常量从 ./db/queries 重新导出，保持旧 import 路径（@/lib/agent）可用
export type { Task, TasksByDate, Importance, Category, TaskSegment, Report };
...
- addTask：批量新增任务。传入 tasks 数组，每条含 title、importance、category、date?（默认今天）、segmentId?（归属任务段）。仅允许今天及以后的日期。
- toggleTask：切换某天某任务的完成状态。
- updateTask：批量修改任务标题。传入 updates 数组，每条含 id、title、date?（任务所属日期，默认今天）。
...
- generateReport：生成周报/月报/阶段报。传入 type、title、periodStart、periodEnd、segmentId?、summary（文字复盘）、planTasks（下周期规划任务）。工具内部自动加载周期任务并计算完成率、五育分布、心理绿芽指数（0-100 分五维加权），你无需自己算指标。同一周期只生成一次。返回 report（含 metrics 与 plan）与 reports 列表。
...
- 用户要求生成周报/月报/阶段报，或用户说「生成报告」「出个周报」「总结一下这周/这个月/这个阶段」：
  · 周报：周期为本周一到本周日（当天须为周末且当天任务全部完成）；月报：当月 1 日到当月最后一天（当天须为月末且当天任务全部完成）；阶段报：对应任务段的 startDate 到 endDate（当天须为段末且当天任务全部完成）。
  · 按以下流程执行报告生成工作流。
- 用户给的日期或任务信息不足以执行时，先用一句话反问澄清，不要盲目猜测或编造。

报告生成工作流（用户请求生成周报/月报/阶段报时）：
1. 确定周期范围（periodStart / periodEnd）与 type（weekly/monthly/stage）：
   · weekly：本周一 ~ 本周日
   · monthly：本月 1 日 ~ 本月最后一天
   · stage：用户指定任务段的 startDate ~ endDate，并带上 segmentId
2. 调用 generateReport，传入：
   · type / title（如「第 N 周周报」「X 月月报」「暑假阶段报」）
   · periodStart / periodEnd / segmentId?
   · summary：你对本周期的 markdown 文字复盘，应包含：
       - 完成情况总览（多少任务、完成多少、完成率）
       - 亮点：哪些方面做得好（如某育完成率高、坚持了某事）
       - 不足：哪些方面欠缺（如某育任务少或完成率低、某天任务堆积）
       - 心理绿芽指数解读：总分多少，哪几个维度强/弱，给出温暖的成长鼓励
       - 可改进点：针对不足给出 1-2 条具体可执行建议
   · planTasks：下周期规划任务数组，根据本周完成情况优化：
       - 补足本周欠缺的五育维度（多安排缺的类别）
       - 保持已完成较好的维度
       - 任务要具体可执行（有明确内容/时长/数量）
       - 适当加入缓冲微任务（体育/美育/劳育，3-10 分钟）保持平衡
       - 日期分布在下周期范围内（如下周一~下周日）
3. 工具返回 { report, reports }：
   · report.metrics 含完成率、五育分布、心理绿芽指数（total + 各维度 score）——这是工具自动算的，你在 summary 里引用这些数字即可
   · 成功后简要告知用户报告已生成，可在「报告」入口查看，并复述心理绿芽指数总分与一两句亮点
   · 若返回 error（如已生成过），如实告知用户该周期已有报告
4. 不要自己计算完成率/指数等数字，工具会算；你在 summary 里引用返回的 report.metrics 数字即可。
 ...
 * @param preferences 用户自定义 Agent 配置（角色 / 技能），可选
    ...
    tools: {
      searchTasks: tool({
        ...
        execute: async ({ query, date }) => {
          const byDate = date
            ? { [date]: await loadDayTasks(userId, date) }
            : await loadTasksByDate(userId);
      ...
      getTasks: tool({
        ...
        execute: async ({ date }) => {
          const d = date ?? today;
          const tasks = await loadDayTasks(userId, d);
          const all = await loadTasksByDate(userId);
          return {
            date: d,
            tasks,
      ...
      addTask: tool({
        description:
          "批量新增任务。传入 tasks 数组，每条含 title、importance、category、date?（默认今天）、segmentId?（归属任务段）。仅允许今天及以后的日期，过去日期会返回错误。无需先调用 getTasks。",
        inputSchema: z.object({
          tasks: z
            .array(
              z.object({
          ...
          for (const t of items) {
            if (!isFutureOrToday(t.date)) {
              return {
                ...
                tasksByDate: await loadTasksByDate(userId),
              };
            }
          }
          const created = await insertTasks(userId, items);
          return {
            created,
            tasksByDate: await loadTasksByDate(userId),
          };
      ...
      toggleTask: tool({
        ...
        execute: async ({ id, date }) => {
          ...
          const existing = await findTaskByIdAndDate(userId, id, d);
          if (!existing) {
            return {
          ...
          const updated = await setTaskDone(userId, id, !existing.done);
          const tasks = await loadDayTasks(userId, d);
          return {
            date: d,
            task: updated,
      ...
      updateTask: tool({
        description:
          "批量修改任务标题。传入 updates 数组，每条含 id、title、date?（任务所属日期，默认今天）。仅允许今天及以后的日期。",
        inputSchema: z.object({
          updates: z
            .array(
              z.object({
                id: z.string().describe("要修改的任务 id"),
                title: z.string().describe("新的任务标题"),
                date: z
                  .string()
                  .optional()
        ...
        execute: async ({ updates }) => {
          ...
          for (const u of updates) {
            ...
            const existing = await findTaskByIdAndDate(userId, u.id, d);
            if (!existing) {
              results.push({
                task: null,
            ...
            const updated = await setTaskTitle(userId, u.id, u.title);
      ...
      moveTask: tool({
        description:
          "批量移动任务到另一天。传入 moves 数组，每条含 id、to（目标日期）。源日期自动查找，目标日期必须是今天及以后。",
        inputSchema: z.object({
          moves: z
            .array(
              z.object({
        ...
        execute: async ({ moves }) => {
          ...
          for (const m of moves) {
            ...
            const existing = await findTaskById(userId, m.id);
            if (!existing) {
              results.push({
                task: null,
                from: "",
            ...
            if (!isFutureOrToday(existing.date)) {
              results.push({
                task: null,
                from: existing.date,
                to: m.to,
                error: `任务在 ${existing.date}（过去日期），不能移动过去的任务`,
              });
              continue;
            }
            ...
            const updated = await setTaskDate(userId, m.id, m.to);
      ...
      deleteTask: tool({
        description:
          "删除某天某条任务。不传 date 默认今天。仅允许今天及以后的日期。",
        inputSchema: z.object({
        ...
        execute: async ({ name, startDate, endDate, description }) => {
          if (startDate > endDate) {
            return {
              ...
              segments: await loadSegments(userId),
            };
          }
          const result = await createSegment(userId, {
            name,
            startDate,
            endDate,
            description,
      ...
      updateTaskSegment: tool({
        ...
        execute: async ({ id, name, startDate, endDate, description }) => {
          ...
          const existing = await findSegmentById(userId, id);
          if (!existing) {
            return {
          ...
          const updated = await updateSegment(userId, id, {
            name,
            startDate,
            endDate,
            description,
      ...
      generateReport: tool({
        description:
          "生成周报/月报/阶段报。根据周期内的全部任务自动计算完成率、五育分布、四象限分布与「心理绿芽指数」（0-100 分五维加权）。你只需提供周期范围、AI 文字总结与下周期规划任务。同一周期只生成一次，重复生成会返回 error。返回的报告含结构化 metrics 与 plan，客户端据此渲染复盘看板、雷达图与下周规划。",
        inputSchema: z.object({
          type: z
            .enum(["weekly", "monthly", "stage"])
            .describe("报告类型：weekly=周报 / monthly=月报 / stage=阶段报"),
          title: z
            .string()
            .describe("报告标题，如「第 26 周周报」或「暑假阶段报」"),
          periodStart: z.string().describe("周期开始日期 YYYY-MM-DD（含当天）"),
          periodEnd: z
            .string()
            .describe("周期结束日期 YYYY-MM-DD（含当天，即周末/月末/阶段末）"),
          segmentId: z
            .string()
            .optional()
            .describe("阶段报对应的任务段 id，周报/月报不传"),
          summary: z
            .string()
            .describe(
              "你对本周期的文字复盘总结（markdown），包括完成情况亮点、不足、可改进点。会展示给用户看。",
            ),
          planTasks: z
            .array(
              z.object({
                title: z.string().describe("下周期规划任务标题"),
                importance: z
                  .enum([
                    "重要且紧急",
                    "重要但不紧急",
                ...
                date: z
                  .string()
                  .describe("计划安排到下周期的哪一天 YYYY-MM-DD"),
              }),
            )
            .min(1)
            .describe(
              "下周期规划任务列表（供「应用」按钮一键创建），根据本周完成情况优化，保持五育与重要度平衡",
            ),
        }),
        execute: async ({
          type,
          ...
          if (periodStart > periodEnd) {
            return {
              ...
              reports: await loadReports(userId),
            };
          }
          // 同周期去重：同 type + 同 periodEnd 视为已生成
          const exists = await hasReportForPeriod(
            userId,
            type as ReportType,
            periodEnd,
          );
          if (exists) {
            return {
              error: `该周期已生成过${type === "weekly" ? "周报" : type === "monthly" ? "月报" : "阶段报"}（periodEnd=${periodEnd}）`,
              reports: await loadReports(userId),
            };
          }
