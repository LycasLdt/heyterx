...
export function computeMessageTaskDiff(
  ...
  for (const part of msg.parts) {
    ...
    if (p.type === "tool-moveTask") {
      const output = p.output as {
        moved?: Array<{ task: Task | null; from: string; to: string }>;
      };
    ...
    if (p.type === "tool-deleteTask") {
      const input = p.input as { date?: string };
      const output = p.output as { task?: Task | null };
      if (!output.task) continue;
      push(input.date ?? today, { kind: "deleted", title: output.task.title });
      continue;
    }

    if (p.type === "tool-toggleTask") {
      const output = p.output as { date?: string; task?: Task | null };
      if (!output.task) continue;
      push(output.date ?? today, {
        kind: "toggled",
        title: output.task.title,
        detail: output.task.done ? "标记完成" : "取消完成",
      });
      continue;
    }
  }
